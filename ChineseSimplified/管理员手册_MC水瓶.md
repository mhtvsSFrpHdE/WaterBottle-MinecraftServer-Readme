...
### 步骤3:
用离线模式以新玩家的游戏名**进入游戏一次**，会显示不在白名单错误。  
接下来[添加玩家到白名单](#commonCommandWhitelist)，  
用新玩家的游戏名**再次**进入游戏一次触发新玩家自动初始化**之后**退出游戏，

解释：  
离线模式和正版玩家的重要区别在识别码不同，  
当一个离线模式玩家**从来没进过服务器**时直接添加白名单，  
会导致添加白名单后也出现不在白名单错误。  
原因是正版玩家识别码由MOJANG管理，离线模式每个服务器自己管理。  
从来没进过服务器的玩家会**先**向MOJANG查询正版识别码，  
而添加白名单**之前**提前以离线模式进入一次就会生成自己的识别码。  
两种不同渠道获得的识别码常常不相等，因此即使玩家名相同，  
服务器也可能因此认为这是不同的两个人而导致白名单添加失败。
